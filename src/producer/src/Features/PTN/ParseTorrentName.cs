...
public class ParseTorrentName : IParseTorrentName
{
    ...
    public ParsedTorrentResult Parse(string title)
    {
        ...
        var parsedResult = new ParsedTorrentResult
        {
            ...
            Unrated = result.TryGetValue(ResultKeys.Unrated, out var unratedValue) && (bool) unratedValue,
            Proper = result.TryGetValue(ResultKeys.Proper, out var properValue) && (bool) properValue,
            Repack = result.TryGetValue(ResultKeys.Repack, out var repackValue) && (bool) repackValue,
            Convert = result.TryGetValue(ResultKeys.Convert, out var convertValue) && (bool) convertValue,
            Hardcoded = result.TryGetValue(ResultKeys.Hardcoded, out var hardcodedValue) && (bool) hardcodedValue,
            Retail = result.TryGetValue(ResultKeys.Retail, out var retailValue) && (bool) retailValue,
            Remastered = result.TryGetValue(ResultKeys.Remastered, out var remasteredValue) && (bool) remasteredValue,
            Complete = result.TryGetValue(ResultKeys.Complete, out var completeValue) && (bool) completeValue,
