...
public class HdrHandlers : IPtnHandler
{
    public void RegisterHandlers(IParseTorrentName parser)
    {
        ...
        parser.AddHandler(ResultKeys.Hdr, new Regex(@"\bHDR(?:10)?\b", RegexOptions.IgnoreCase), Transformers.UniqConcat(Transformers.Value(ResultKeys.Hdr)), new() { Remove = true, SkipIfAlreadyFound = false });
    }
}